...
html2canvas-batch does not get around content policy restrictions set by your browser. Drawing images that reside outside of 
the [origin](https://developer.mozilla.org/en-US/docs/Web/Security/Same-origin_policy) of the current page [taint the 
canvas](https://developer.mozilla.org/en-US/docs/Web/HTML/CORS_enabled_image#What_is_a_tainted_canvas) that they are drawn upon. If the canvas gets tainted, it cannot be read anymore. As such, html2canvas-batch implements 
...
As each CSS property needs to be manually coded to render correctly, html2canvas-batch will *never* have full CSS support. 
...
## How do I get html2canvas-batch to work in a browser extension?
You shouldn't use html2canvas-batch in a browser extension. Most browsers have native support for capturing screenshots from 
